...
inline static void LOG(const std::string_view& error) {
    std::cerr << error;
}

Kalman::Kalman(FilterModel& filter, MeasureModel& measure) :
    A_{std::move(filter.A_)}, B_{std::move(filter.B_)}, P_{std::move(filter.P_)}, Q_{std::move(filter.Q_)},
    x_{std::move(filter.x_)}, u_{std::move(filter.u_)}, H_{std::move(measure.H_)}, R_{std::move(measure.R_)},
    S_{std::move(measure.S_)}, K_{std::move(measure.K_)}, z_{std::move(measure.z_)}, y_{std::move(measure.z_)},
    inputs_{filter.inputs_}, states_{filter.inputs_}, measurements_{measure.measurements_} {
...
// using std::move to explicitly cast to rvalue and force move assingment operator, as even though operators (like
// functions) return temporaries, which are rvalues, you need explicit cast to perform move!
void Kalman::predict() {
    if (!isInitialized_) {
